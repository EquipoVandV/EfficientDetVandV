...
def invert_affine(metas: Union[float, list, tuple], preds):
    for i in range(len(preds)):
        if len(preds[i]['bbox']) == 0:
            continue
        else:
            if metas is float:
                preds[i]['bbox'][:, [0, 2]] = preds[i]['bbox'][:, [0, 2]] / metas
                preds[i]['bbox'][:, [1, 3]] = preds[i]['bbox'][:, [1, 3]] / metas
            else:
                new_w, new_h, old_w, old_h, padding_w, padding_h = metas[i]
                preds[i]['bbox'][:, [0, 2]] = preds[i]['bbox'][:, [0, 2]] / (new_w / old_w)
                preds[i]['bbox'][:, [1, 3]] = preds[i]['bbox'][:, [1, 3]] / (new_h / old_h)
    return preds
...
def preprocess(image_path, max_size=512, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    ...
    ori_imgs=[image_path]
    # ori_imgs =[cv2.cvtColor((i), cv2.COLOR_BGR2RGB) for i in ori_imgs]  #BGR to RGB
...
def display(preds, imgs, obj_list, imshow=True, imwrite=False):
    for i in range(len(imgs)):
        ...
        for j in range(len(preds[i]['bbox'])):
            (x1, y1, x2, y2) = preds[i]['bbox'][j].astype(np.int)
